...
export function GoogleSignInButton({ redirectError }: { redirectError?: string }) {
  ...
  async function handleClick() {
    ...
    try {
      ...
      const { error: oauthError } = await supabase.auth.signInWithOAuth({
        provider: "google",
        options: {
          redirectTo: `${window.location.origin}/auth/callback?next=%2Fadmin`,
          // Always show Google's account picker instead of silently reusing
          // whichever Google account is already signed in in this browser.
  ...
  return (
    <div>
      <button
        ...
        onClick={handleClick}
        disabled={pending}
        className="flex h-12 w-full items-center justify-center gap-3 border border-line-strong bg-ivory text-[14px] font-medium text-ink transition-opacity hover:opacity-90 disabled:opacity-50"
      >
        <GoogleIcon />
